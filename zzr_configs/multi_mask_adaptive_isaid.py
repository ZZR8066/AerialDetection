...
model = dict(
    ...
    mask_rroi_normal_extractor=dict(
        type='ARboxSingleRoIExtractor',
        roi_layer=dict(type='ARoIAlignRotated', out_size=14, sample_num=2),
        out_channels=256,
        ratio_max=1.0,
        featmap_strides=[4, 8, 16, 32]),
    mask_rroi_adaptive_extractor=dict(
        type='ARboxSingleRoIExtractor',
        roi_layer=dict(type='ARoIAlignRotated', out_size=14, sample_num=2),
        out_channels=256,
        ratio_max=6.0,
...
log_level = 'INFO'
work_dir = '/disk1/zzr/work_dirs/multi_mask_adaptive_isaid'
load_from = None
resume_from = None
workflow = [('train', 1)]
