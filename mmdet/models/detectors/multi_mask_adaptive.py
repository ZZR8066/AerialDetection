...
from .zzr_function import (test_rroi2roi, draw_gt_obboxes, mask2obb_mask, 
                        det_rbboxes2det_bboxes, rbboxes2rects, rrois2rois)

@DETECTORS.register_module
class MultiMaskAdaptive(BaseDetectorNew, RPNTestMixin, MaskTestMixin):
    ...
    def __init__(self,
                 ...
                 mask_head=None,
                 mask_rroi_adaptive_extractor=None,
                 mask_rroi_normal_extractor=None,
                 mask_rhead=None,
                 threshold=5,
                 train_cfg=None,
                 test_cfg=None,
        ...
        super(MultiMaskAdaptive, self).__init__()
        ...
        # obb mask
        self.mask_rroi_adaptive_extractor = builder.build_roi_extractor(mask_rroi_adaptive_extractor)
        self.mask_rroi_normal_extractor = builder.build_roi_extractor(mask_rroi_normal_extractor)
    ...
    def init_weights(self, pretrained=None):
        super(MultiMaskAdaptive, self).init_weights(pretrained)
    ...
    def forward_train(self,
        ...
        # adaptive pool
        use_adaptive = ((pos_rrois[:,3] / pos_rrois[:,4]) > self.threshold)
        assert use_adaptive.shape[0] != 0
        use_adaptive_ratio = 1.0*use_adaptive.sum()/use_adaptive.shape[0]
        loss_mask_obb_adaptive = 0.0
        loss_mask_obb_normal = 0.0
        if use_adaptive.any():
            mask_feats = self.mask_rroi_adaptive_extractor(
                        x[:self.mask_rroi_adaptive_extractor.num_inputs], pos_rrois[use_adaptive])
            mask_pred = self.mask_rhead(mask_feats)
            mask_targets = self.mask_rhead.get_rotate_adaptive_target(
                    sampling_results, gt_masks, self.train_cfg.rcnn[1],
                    self.mask_rroi_adaptive_extractor.ratio_max)
            pos_labels = torch.cat(
                    [res.pos_gt_labels for res in sampling_results])
            loss_mask = self.mask_rhead.loss(mask_pred, mask_targets[use_adaptive],
                                                pos_labels[use_adaptive])
            loss_mask_obb_adaptive = loss_mask.pop('loss_mask')*use_adaptive_ratio
        # normal pool
        if (~use_adaptive).any():
            mask_feats = self.mask_rroi_normal_extractor(
                        x[:self.mask_rroi_normal_extractor.num_inputs], pos_rrois[~use_adaptive])
            mask_pred = self.mask_rhead(mask_feats)
            mask_targets = self.mask_rhead.get_rotate_adaptive_target(
                    sampling_results, gt_masks, self.train_cfg.rcnn[1],
                    self.mask_rroi_normal_extractor.ratio_max)
            loss_mask = self.mask_rhead.loss(mask_pred, mask_targets[~use_adaptive],
                                                pos_labels[~use_adaptive])
            loss_mask_obb_normal = loss_mask.pop('loss_mask')*(1-use_adaptive_ratio)

        lose_mask = dict()
        loss_mask['loss_mask_obb']=loss_mask_obb_adaptive+loss_mask_obb_normal
        losses.update(loss_mask)
        return losses
    ...
    def simple_test_rotate_mask(self, x, img_meta, 
        ...
        if det_rbboxes.shape[0] == 0:
            ...
            use_adaptive = ((mask_rois[:,3] / mask_rois[:,4]) > self.threshold)
            # adaptive part
            mask_feats = self.mask_rroi_adaptive_extractor(
                x[:len(self.mask_rroi_adaptive_extractor.featmap_strides)], mask_rois[use_adaptive])
            if self.with_shared_head:
                mask_feats = self.shared_head(mask_feats)
            mask_pred = self.mask_rhead(mask_feats)
            segm_result_adaptive = self.mask_rhead.get_rotate_seg_masks(mask_pred, 
                    _rects[use_adaptive], det_labels[use_adaptive], self.test_cfg.rcnn, ori_shape,
                    scale_factor, rescale)
            # normal part
            mask_feats = self.mask_rroi_normal_extractor(
                x[:len(self.mask_rroi_normal_extractor.featmap_strides)], mask_rois[~use_adaptive])
            if self.with_shared_head:
                mask_feats = self.shared_head(mask_feats)
            mask_pred = self.mask_rhead(mask_feats)
            segm_result_normal = self.mask_rhead.get_rotate_seg_masks(mask_pred, 
                    _rects[~use_adaptive], det_labels[~use_adaptive], self.test_cfg.rcnn, ori_shape,
                    scale_factor, rescale)
            segm_result = self.merge_segm_result(segm_result_normal, segm_result_adaptive)
        return segm_result
    
    def aug_test(self, imgs, img_metas, proposals=None, rescale=None):
        print('aug_test is not support')
        
    def merge_segm_result(self, segm_result_normal, segm_result_adaptive):
        pass