...
class AFRoITransformerVertex(BaseDetectorNew, RPNTestMixin):
    ''' RoITransformer concat Vertex Regression RCNN '''
    def __init__(self,
                 backbone,
                 neck=None,
                 shared_head=None,
        ...
        super(AFRoITransformerVertex, self).__init__()
        ...
        if vertex_head is not None:
            self.vertex_roi_extractor = builder.build_roi_extractor(
                vertex_roi_extractor)
            self.vertex_head = builder.build_head(vertex_head)
    ...
    def init_weights(self, pretrained=None):
        super(AFRoITransformerVertex, self).init_weights(pretrained)
        ...
        self.vertex_roi_extractor.init_weights()
    ...
    def forward_train(self,
        ...
        with torch.no_grad():
            # import pdb
            # pdb.set_trace()
            rotated_proposal_list = self.rbbox_head.refine_rbboxes(
                rrois, roi_labels, rbbox_pred, pos_is_gts, img_meta
            )
            
        ''' Vertex Regressioin RCNN'''
        if self.with_rbbox:
            bbox_assigner = build_assigner(self.train_cfg.rcnn[1].assigner)
            bbox_sampler = build_sampler(
                self.train_cfg.rcnn[1].sampler, context=self)
        ...
        if self.with_rbbox:
            # (batch_ind, x_ctr, y_ctr, w, h, angle)
            rrois = dbbox2roi([res.bboxes for res in sampling_results])
            rbbox_feats = self.vertex_roi_extractor(x[:self.rbbox_roi_extractor.num_inputs],
                                                   rrois)
            cls_score, rbbox_pred = self.vertex_head(rbbox_feats)
            rbbox_targets = self.vertex_head.get_target_rbbox(sampling_results, gt_obbs,
                                                        gt_labels, self.train_cfg.rcnn[1])
            loss_rbbox = self.vertex_head.loss(cls_score, rbbox_pred, *rbbox_targets)
            for name, value in loss_rbbox.items():
