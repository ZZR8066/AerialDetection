...
class AF_APE_Vertex(BaseDetectorNew, RPNTestMixin):
    ...
    def __init__(self,
        ...
        super(AF_APE_Vertex, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        super(AF_APE_Vertex, self).init_weights(pretrained)
    ...
    def forward_train(self,
        ...
        if self.with_rbbox:
            ...
            for i in range(num_imgs):
                assign_result = bbox_assigner.assign(
                    rotated_proposal_list[i], gt_obbs[i], gt_bboxes_ignore[i],
                    gt_labels[i])
                sampling_result = bbox_sampler.sample(
                    assign_result,
                    rotated_proposal_list[i],
                    torch.from_numpy(gt_obbs[i]).float().to(rotated_proposal_list[i].device),
                    gt_labels[i],
                    feats=[lvl_feat[i][None] for lvl_feat in x])
                sampling_results.append(sampling_result)
