...
from ..detectors.zzr_function import crop_rotate_mask, trans2hbb
...
class MaskIoUHead(nn.Module):
    ...
    def __init__(self,
        ...
        roi_feat_size = (roi_feat_size_h, roi_feat_size_w)
    ...
    def get_target_hbb(self, sampling_results, gt_masks, mask_pred, mask_targets,
                   rcnn_train_cfg):
        pos_proposals = [res.pos_bboxes for res in sampling_results]
        pos_proposals = trans2hbb(pos_proposals)
        pos_assigned_gt_inds = [
            res.pos_assigned_gt_inds for res in sampling_results
        ]
    ...
    def get_rotate_area_ratio(self, pos_proposals_list, pos_assigned_gt_inds_list, gt_masks_list):
        area_ratios_list = []
        for i in range(len(pos_proposals_list)):
            ...
            if num_pos > 0:
                area_ratios = []
                proposals_np = pos_proposals.data.cpu().numpy()
                pos_assigned_gt_inds = pos_assigned_gt_inds.data.cpu().numpy()
                # compute mask areas of gt instances (batch processing for speedup)
                gt_instance_mask_area = gt_masks.sum((-1, -2))
                for j in range(num_pos):
                    gt_mask = gt_masks[pos_assigned_gt_inds[j]]
                    
                    #crop the gt mask inside the proposals
                    x,y,w,h,theta = proposals_np[j, :]
                    gt_mask_in_proposal = crop_rotate_mask(gt_mask,x,y,w,h,theta)
                    
                    ratio = gt_mask_in_proposal.sum() / (
                        gt_instance_mask_area[pos_assigned_gt_inds[j]] + 1e-7)
                    area_ratios.append(ratio)
                area_ratios = torch.from_numpy(np.stack(area_ratios)).float().to(
                    pos_proposals.device)   
            else:
        ...
        return torch.cat(list(area_ratios_list))

    def get_rotate_target(self, sampling_results, gt_masks_list, 
        mask_pred_rotate, mask_targets_rotate, rcnn_train_cfg):
        pos_proposals_list = [res.pos_bboxes for res in sampling_results]
        pos_assigned_gt_inds_list = [
            res.pos_assigned_gt_inds for res in sampling_results
        ]
        area_ratios = self.get_rotate_area_ratio(pos_proposals_list, 
            pos_assigned_gt_inds_list, gt_masks_list)
        assert mask_targets_rotate.size(0) == area_ratios.size(0)
        
        mask_pred_rotate = (mask_pred_rotate > rcnn_train_cfg.mask_thr_binary).float()
        mask_pred_rotate_areas = mask_pred_rotate.sum((-1,-2))
        
        # mask_pred and mask_targets are binary maps
        overlap_areas = (mask_pred_rotate * mask_targets_rotate).sum((-1, -2))
        
        # compute the mask area of the whole instance
        gt_full_areas = mask_targets_rotate.sum((-1, -2)) / (area_ratios + 1e-7)
        
        rotate_mask_iou_targets = overlap_areas / (
            mask_pred_rotate_areas + gt_full_areas - overlap_areas)
        return rotate_mask_iou_targets
